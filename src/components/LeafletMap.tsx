...
// @ts-expect-error: No types for @changey/react-leaflet-markercluster
...
function LocationButton() {
  ...
  const handleLocationFound = (position: GeolocationPosition) => {
    ...
    if (accuracy > 100) {
      alert(`Upozornění: Přesnost lokace je nízká (±${Math.round(accuracy)}m). Pro lepší přesnost:\n` +
  ...
  const handleLocationError = (error: GeolocationPositionError) => {
    ...
    switch (error.code) {
      case 1:
        errorMessage = 'Přístup k poloze byl zamítnut. Povolte přístup k poloze v nastavení prohlížeče.';
        break;
      case 2:
        errorMessage = 'Polohu nelze určit. Zkontrolujte, zda máte zapnuté polohové služby.';
        break;
      case 3:
      ...
      >
        {loading ? (
          ...
          <span className="flex items-center">
            ...
            <span className="ml-2">{t('find_my_location') || 'Moje poloha'}</span>
          </span>
        )}
      </button>
      {userLocation && (
        <Marker position={userLocation} icon={blueIcon}>
          <Popup>
            <div>
              ...
              {locationAccuracy && (
                <span className={locationAccuracy > 100 ? 'text-orange-500' : 'text-green-500'}>
                  Přesnost: ±{Math.round(locationAccuracy)}m
                  {locationAccuracy > 100 && (
                    <div className="text-xs mt-1 text-gray-600">
                      Pro lepší přesnost:<br />
                      • Povolte přesnou polohu<br />
                      • Použijte zařízení s GPS<br />
                      • Buďte venku/u okna
                    </div>
                  )}
                </span>
...
export default function LeafletMap({ selectedWasteType }: { selectedWasteType?: string }) {
  const { t } = useTranslation();
  const [features, setFeatures] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const mapRef = useRef<L.Map | null>(null);
  const [wasteType, setWasteType] = useState<string | undefined>(selectedWasteType);
  ...
  const filteredFeatures = wasteType
    ? features.filter(f => f.properties?.komodita_odpad_separovany === wasteType)
    : features;
  ...
  return (
    <MapControlContext.Provider value={contextValue}>
      <div className="h-[600px] bg-gray-200 rounded-lg dark:bg-gray-900 relative">
            ...
            {filteredFeatures.map((feature, idx) => {
              ...
              return (
                <Marker key={idx} position={[lat, lng]} icon={greenIcon}>
                  <Popup>
                    {t('waste_type')}: {feature.properties?.komodita_odpad_separovany || '-'}<br />
        ...
        {loading && (
          <p className="text-center text-gray-500 mt-4 dark:text-gray-300">{t('loading_map')}</p>
        )}
        {error && (
